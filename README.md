...
- 新建、编辑或临时方案切换为“不安排休息”时，休息字段及两项休息自动衔接复选框留在原位并明确变灰，不改变窗口高度和表单对齐；连续专注选项同时启用。
...
最终在 `dist/` 下生成独立目录和 ZIP 压缩包。发布包根目录同时包含标准命名的
`Uninstall.exe`：运行后可卸载当前程序，并永久删除 FocusFlow 的本地数据库、
设置、托管提示音和自动备份。卸载前会再次确认；程序仍在运行时不会执行删除。
